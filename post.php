...
if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
        echo "<embed  style='margin-top:40px;' src='administration/up/".$row["id"]."/post.swf'>";
        //echo "<div style=' color:#ffffff; position:relative;'><ul style=' margin: 1px 170px; padding:1px 35px; background-color:#ff1c37; max-width:700px;'><li><p>".$row["id_pub"]." | ".$row["num_tel"]." | ".$row["publisher_username"]." | ".$row["date_res"]." - ".$row["time_res"]." | ".$row["dep_arr"]."<a style='text-decoration:none; color:#ffffff; float:right; padding :0px 5px;' href='delete.php?id_post=".$row['id']"><i class='fa fa-times'></i> </a></p></li></ul></div>";
    }
} else {
    echo "0 results";
}
